...
export const api = {
  ...
  sharedWorkspace: {
    state: () => authenticatedFetch('/api/shared-workspace/state'),
    setState: (payload) =>
      authenticatedFetch('/api/shared-workspace/state', {
        method: 'PUT',
        body: JSON.stringify(payload),
      }),
    registerContinuation: (payload) =>
      authenticatedFetch('/api/shared-workspace/continuation', {
        method: 'POST',
        body: JSON.stringify(payload),
      }),
    bindings: () => authenticatedFetch('/api/shared-workspace/bindings'),
    binding: ({ channelType, channelId, userId = '' }) =>
      authenticatedFetch(
        `/api/shared-workspace/binding?channelType=${encodeURIComponent(channelType)}&channelId=${encodeURIComponent(channelId)}&userId=${encodeURIComponent(userId)}`,
      ),
    upsertBinding: (payload) =>
      authenticatedFetch('/api/shared-workspace/binding', {
        method: 'PUT',
        body: JSON.stringify(payload),
      }),
    createMessageEvent: (payload) =>
      authenticatedFetch('/api/shared-workspace/message-event', {
        method: 'POST',
        body: JSON.stringify(payload),
      }),
    messageEvents: ({ channelType, channelId, userId = '', limit = 50 }) =>
      authenticatedFetch(
        `/api/shared-workspace/message-events?channelType=${encodeURIComponent(channelType)}&channelId=${encodeURIComponent(channelId)}&userId=${encodeURIComponent(userId)}&limit=${encodeURIComponent(limit)}`,
      ),
    ackMessageEvents: (ids) =>
      authenticatedFetch('/api/shared-workspace/message-events/ack', {
        method: 'POST',
