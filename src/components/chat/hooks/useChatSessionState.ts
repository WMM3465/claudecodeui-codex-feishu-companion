...
export function useChatSessionState({
  ...
  const getRuntimeSessionId = useCallback((session: ProjectSession | null | undefined) => {
    if (!session) return '';
    return (typeof session.__activeSessionId === 'string' && session.__activeSessionId) || session.id || '';
  }, []);
  ...
  const displaySessionId = selectedSession?.id || currentSessionId || null;
  const activeSessionId = getRuntimeSessionId(selectedSession) || currentSessionId || null;
  const [pendingUserMessage, setPendingUserMessage] = useState<ChatMessage | null>(null);

  // Tell the store which visible thread we're viewing so only the root thread rerenders.
  const prevDisplayForStoreRef = useRef<string | null>(null);
  if (displaySessionId !== prevDisplayForStoreRef.current) {
    prevDisplayForStoreRef.current = displaySessionId;
    sessionStore.setActiveSession(displaySessionId);
  }

  // When a visible thread ID arrives and we have a pending user message, flush it there.
  const prevActiveSessionRef = useRef<string | null>(null);
  if (displaySessionId && displaySessionId !== prevActiveSessionRef.current && pendingUserMessage) {
    const prov = (localStorage.getItem('selected-provider') as SessionProvider) || 'claude';
    const normalized = chatMessageToNormalized(pendingUserMessage, displaySessionId, prov);
    if (normalized) {
      sessionStore.appendRealtime(displaySessionId, normalized);
    }
    setPendingUserMessage(null);
  }
  prevActiveSessionRef.current = displaySessionId;

  const storeMessages = displaySessionId ? sessionStore.getMessages(displaySessionId) : [];
  ...
  const addMessage = useCallback((msg: ChatMessage) => {
    ...
    const prov = (localStorage.getItem('selected-provider') as SessionProvider) || 'claude';
    const normalized = chatMessageToNormalized(msg, displaySessionId, prov);
    if (normalized) {
      sessionStore.appendRealtime(displaySessionId, normalized);
    }
  }, [displaySessionId, sessionStore]);

  const clearMessages = useCallback(() => {
    if (!displaySessionId) return;
    sessionStore.clearRealtime(displaySessionId);
  }, [displaySessionId, sessionStore]);
  ...
  useEffect(() => {
    ...
    const runtimeSessionId = getRuntimeSessionId(selectedSession) || selectedSession.id;
    const sessionKey = `${selectedSession.id}:${runtimeSessionId}:${selectedProject.name}:${provider}`;
    ...
    const sessionChanged = currentSessionId !== null && currentSessionId !== runtimeSessionId;
    if (sessionChanged) {
      resetStreamingState();
      pendingViewSessionRef.current = null;
      setClaudeStatus(null);
    ...
    if (ws) {
      sendMessage({ type: 'check-session-status', sessionId: runtimeSessionId, provider });
    }
    ...
    sessionStore.fetchFromServer(selectedSession.id, {
      ...
      if (slot) {
        ...
        } else if ((selectedSession.__provider || provider) === 'codex') {
          setTokenBudget(null);
        }
      }
    ...
    selectedProject,
    selectedSession?.id,
    selectedSession?.__activeSessionId,
    sendMessage,
    ws,
    sessionStore,
    currentSessionId,
  ...
  useEffect(() => {
    ...
    const reloadExternalMessages = async () => {
      try {
        ...
        if (!isLoading) {
          const slot = await sessionStore.refreshFromServer(selectedSession.id, {
            provider: (selectedSession.__provider || provider) as SessionProvider,
            projectName: selectedProject.name,
            projectPath: selectedProject.fullPath || selectedProject.path || '',
          });
  ...
  useEffect(() => {
    ...
    const refreshCurrentCodexSession = async () => {
      ...
      try {
        ...
        console.error('Error polling codex shared session:', error);
      }
    };

    const interval = window.setInterval(() => {
      void refreshCurrentCodexSession();
    }, 3000);
  ...
  }, [isLoading, selectedProject, selectedSession, sessionStore]);
  ...
  useEffect(() => {
    const runtimeSessionId =
      getRuntimeSessionId(selectedSession) ||
      currentSessionId ||
      selectedSession?.id ||
      null;
    if (!runtimeSessionId || !processingSessions) return;

    const shouldBeProcessing = processingSessions.has(runtimeSessionId);
  ...
  }, [currentSessionId, getRuntimeSessionId, isLoading, processingSessions, selectedSession]);
