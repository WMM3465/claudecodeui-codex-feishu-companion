...
function sanitizeContinuationWrappedUserContent(content: string): string {
  const raw = String(content || '');
  const wrappedMatch = raw.match(
    /<context_summary>\s*[\s\S]*?<\/context_summary>[\s\S]*?<new_request>\s*([\s\S]*?)\s*<\/new_request>/i,
  );
  if (wrappedMatch?.[1]) {
    return wrappedMatch[1].trim();
  }

  const chineseWrappedMatch = raw.match(/<新的请求>\s*([\s\S]*?)\s*<\/新的请求>/i);
  if (chineseWrappedMatch?.[1]) {
    return chineseWrappedMatch[1].trim();
  }
    ...
    switch (msg.kind) {
      case 'text': {
        ...
        if (msg.role === 'user') {
          ...
          if (taskNotifMatch) {
            ...
            converted.push({
              type: 'user',
              content: unescapeWithMathProtection(
                decodeHtmlEntities(sanitizeContinuationWrappedUserContent(content)),
              ),
              timestamp: msg.timestamp,
            });
          }
      ...
      case 'task_notification':
        converted.push({
          ...
          isContinuationSummary: Boolean(msg.isContinuationSummary),
