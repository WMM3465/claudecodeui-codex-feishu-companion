...
const commandDescriptionMap: Record<string, string> = {
  '/help': '显示 Claude Code 的帮助文档',
  '/clear': '清空当前对话记录',
  '/model': '切换或查看当前 AI 模型',
  '/cost': '查看 token 使用量和费用信息',
  '/memory': '查看或编辑当前记忆',
  '/config': '打开设置和配置',
  ...
  '/undo': '撤销最近一次操作',
  '/redo': '恢复刚刚撤销的操作',
  '/compact': '压缩当前上下文，减少占用',
  '/agents': '查看可用代理或切换代理',
  '/sessions': '查看最近的会话列表',
  '/new': '新建一个会话',
};

const rawDescriptionMap: Record<string, string> = {
  'Show help documentation for Claude Code': '显示 Claude Code 的帮助文档',
  'Clear the conversation history': '清空当前对话记录',
  'Switch or view the current AI model': '切换或查看当前 AI 模型',
  'Display token usage and cost information': '查看 token 使用量和费用信息',
  'View or edit memory': '查看或编辑当前记忆',
  'Open settings and configuration': '打开设置和配置',
  'Show or adjust the current permission mode': '查看或调整当前权限模式',
  'Show the current session status': '查看当前会话状态',
  'Initialize this project for Claude Code': '为当前项目初始化工作配置',
  'Review current changes': '对当前改动执行审查',
  'Rewind the conversation to a previous state': '回退对话到之前的状态',
  'Undo the last action': '撤销最近一次操作',
  'Redo the last undone action': '恢复刚刚撤销的操作',
  'Compact the current context to save tokens': '压缩当前上下文，减少占用',
  'List available agents or switch agent': '查看可用代理或切换代理',
  'List recent sessions': '查看最近的会话列表',
  'Create a new session': '新建一个会话',
};
...
const getTranslatedDescription = (command: CommandMenuCommand) =>
  commandDescriptionMap[command.name] ||
  (command.description ? rawDescriptionMap[command.description] || command.description : undefined);

const getTranslatedMetadataType = (metadataType?: string) => {
  if (!metadataType) {
    return metadataType;
  }

  const normalizedType = metadataType.toLowerCase();
  if (normalizedType === 'builtin' || normalizedType === 'built-in') {
    return '内置';
  }
...
export default function CommandMenu({
  ...
  const namespaceLabels: Record<string, string> = {
    frequent: t('commandMenu.namespaces.frequent'),
    builtin: t('commandMenu.namespaces.builtin'),
    project: t('commandMenu.namespaces.project'),
    user: t('commandMenu.namespaces.user'),
    other: t('commandMenu.namespaces.other'),
  };
      ...
      {orderedNamespaces.map((namespace) => (
        <div key={namespace} className="command-group">
          ...
          {(groupedCommands[namespace] || []).map((command) => {
            ...
            const translatedDescription = getTranslatedDescription(command);
            return (
              <div
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className={`flex items-center gap-2 ${translatedDescription ? 'mb-1' : 'mb-0'}`}>
                    ...
                    {command.metadata?.type && (
                      <span className="command-metadata-badge rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-gray-500 dark:bg-gray-700 dark:text-gray-300">
                        {getTranslatedMetadataType(String(command.metadata.type))}
                      </span>
                    )}
                  </div>
