...
const MessageComponent = memo(({ message, prevMessage, createDiff, onFileOpen, onShowSettings, onGrantToolPermission, autoExpandTools, showRawParameters, showThinking, selectedProject, provider }: MessageComponentProps) => {
  ...
  if (message.isTaskNotification && message.isContinuationSummary) {
    return (
      <div
        ref={messageRef}
        data-message-timestamp={message.timestamp || undefined}
        className="chat-message task_notification px-3 sm:px-0"
      >
        <div className="w-full">
          <details className="mx-auto my-4 w-full rounded-xl border border-gray-700/60 bg-gray-900/40">
            <summary className="cursor-pointer list-none px-4 py-3 text-center text-xs tracking-wide text-gray-400 marker:hidden">
              压缩上下文摘要
            </summary>
            <div className="border-t border-gray-700/50 px-4 py-3 text-sm text-gray-300">
              <Markdown className="prose prose-sm max-w-none dark:prose-invert">
                {String(message.continuationSummary || '')}
              </Markdown>
            </div>
          </details>
        </div>
  ...
  return (
    ...
    >
      {message.type === 'user' ? (
        ...
        /* Compact task notification / continuation summary on the left */
        <div className="w-full">
          {message.isContinuationSummary ? (
            <details className="mx-auto my-4 w-full rounded-xl border border-gray-700/60 bg-gray-900/40">
              <summary className="cursor-pointer list-none px-4 py-3 text-center text-xs tracking-wide text-gray-400 marker:hidden">
                压缩上下文摘要
              </summary>
