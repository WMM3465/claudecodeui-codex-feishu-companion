...
const CodeBlock = ({ node, inline, className, children, ...props }: CodeBlockProps) => {
  ...
  const summaryLabel = language && language !== 'text'
    ? `${t('codeBlock.view', { defaultValue: '查看代码' })}（${language}）`
    : t('codeBlock.viewContent', { defaultValue: '查看内容' });
  ...
  return (
    <details className="group my-2 overflow-hidden rounded-lg border border-gray-200/70 bg-gray-50/70 dark:border-gray-700/60 dark:bg-gray-900/30">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2 text-sm text-gray-700 hover:bg-gray-100/80 dark:text-gray-200 dark:hover:bg-gray-800/60">
        ...
        <span className="text-xs text-gray-500 dark:text-gray-400">{t('codeBlock.clickToExpand', { defaultValue: '点击展开' })}</span>
      </summary>
      <div className="group/code relative border-t border-gray-200/70 dark:border-gray-700/60">
        ...
        <button
          ...
          className="absolute right-2 top-2 z-10 rounded-md border border-gray-600 bg-gray-700/80 px-2 py-1 text-xs text-white opacity-0 transition-opacity hover:bg-gray-700 focus:opacity-100 active:opacity-100 group-hover/code:opacity-100"
