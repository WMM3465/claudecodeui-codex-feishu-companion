import { Check, Clock, Edit2, Eye, EyeOff, Trash2, X } from 'lucide-react';
...
type SidebarSessionItemProps = {
  ...
  onToggleHiddenSession: (projectName: string, sessionId: string) => void;
  onDragStartSession?: (projectName: string, sessionId: string) => void;
  t: TFunction;
};
...
export default function SidebarSessionItem({
  ...
  const toggleHiddenSession = () => {
    onToggleHiddenSession(project.name, session.id);
  };

  const handleDragStart = (event: React.DragEvent<HTMLDivElement | HTMLButtonElement>) => {
    event.dataTransfer.setData(
      'application/x-cloudcli-session',
      JSON.stringify({
    ...
    onDragStartSession?.(project.name, session.id);
  };
          ...
          <div className="flex items-center gap-2">
            ...
            {!sessionView.isCursorSession && (
              <div className="ml-1 flex items-center gap-1">
                <button
                  className="flex h-5 w-5 items-center justify-center rounded-md bg-gray-50 opacity-70 transition-transform active:scale-95 dark:bg-gray-900/20"
                  onClick={(event) => {
                    event.stopPropagation();
                    toggleHiddenSession();
                  }}
                  title={isHidden ? t('tooltips.showSession') : t('tooltips.hideSession')}
                >
                  {isHidden ? (
                    <Eye className="h-2.5 w-2.5 text-gray-600 dark:text-gray-400" />
                  ) : (
                    <EyeOff className="h-2.5 w-2.5 text-gray-600 dark:text-gray-400" />
                  )}
                </button>
                <button
