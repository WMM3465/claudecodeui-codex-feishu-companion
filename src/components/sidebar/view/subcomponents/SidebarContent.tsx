...
import { ChevronDown, ChevronRight, Eye, Folder, FolderArchive, MessageSquare, Search } from 'lucide-react';
...
type SidebarContentProps = {
  ...
  onShowSettings: () => void;
  hiddenProjectsExpanded: boolean;
  hiddenProjectEntries: Project[];
  onUnhideProject: (projectName: string) => void;
  onToggleHiddenProjectsSection: () => void;
...
export default function SidebarContent({
  ...
  onUnhideProject,
  onToggleHiddenProjectsSection,
  projectListProps,
  t,
}: SidebarContentProps) {
  const showConversationSearch = searchMode === 'conversations' && searchFilter.trim().length >= 2;
  const hasPartialResults = conversationResults && conversationResults.results.length > 0;
  const [isDragOverHiddenProjects, setIsDragOverHiddenProjects] = useState(false);

  const handleHiddenProjectsDrop = (event: React.DragEvent<HTMLDivElement>) => {
    event.preventDefault();
    setIsDragOverHiddenProjects(false);
    const raw = event.dataTransfer.getData('application/x-cloudcli-project');
    if (!raw) return;
    try {
      const payload = JSON.parse(raw) as { projectName?: string };
      if (!payload.projectName) return;
      projectListProps.onHideProject(payload.projectName);
  ...
  return (
    ...
    >
      ...
      <div className="mx-2 mb-2 flex-shrink-0 rounded-xl border border-primary/40 bg-background/95 p-2 shadow-xl backdrop-blur-md">
        <div className="mb-2 flex items-center gap-2 px-1 text-sm font-semibold text-foreground">
          <FolderArchive className="h-4 w-4 text-primary/80" />
          <span>{t('hiddenProjects.title')}</span>
        </div>
        <div
          className={`rounded-lg border p-2 transition-colors ${
            isDragOverHiddenProjects ? 'border-primary/70 bg-primary/8' : 'border-dashed border-primary/35 bg-muted/20'
          }`}
          onDragOver={(event) => {
            event.preventDefault();
            event.dataTransfer.dropEffect = 'move';
            setIsDragOverHiddenProjects(true);
          }}
          onDragLeave={() => setIsDragOverHiddenProjects(false)}
          onDrop={handleHiddenProjectsDrop}
        >
          <button
            className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-left text-sm font-medium text-foreground transition-colors hover:bg-accent/50"
            onClick={onToggleHiddenProjectsSection}
          >
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground">{hiddenProjectsExpanded ? '收起' : '展开'}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">
                {hiddenProjectEntries.length}
              </span>
              {hiddenProjectsExpanded ? (
          ...
          {hiddenProjectsExpanded && (
            <div className="mt-2 max-h-48 min-h-[72px] space-y-1 overflow-y-auto rounded-lg border border-dashed border-primary/40 bg-muted/10 p-2">
              {hiddenProjectEntries.length === 0 ? (
                <div className="rounded-md px-2 py-3 text-xs text-muted-foreground">
                  <div className="font-medium text-foreground/90">项目隐藏区</div>
                  <div className="mt-1">{t('hiddenProjects.empty')}</div>
                </div>
              ) : (
                hiddenProjectEntries.map((project) => (
                  <div
                    key={project.name}
                    className="flex items-center justify-between rounded-lg px-2 py-2 transition-colors hover:bg-accent/40"
                  >
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium text-foreground" title={project.displayName}>
                        {project.displayName}
                      </div>
                      <div className="truncate text-[11px] text-muted-foreground" title={project.fullPath}>
                        {project.fullPath}
                      </div>
                    </div>
                    <button
                      className="ml-2 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-md border border-emerald-200 bg-emerald-500/10 text-emerald-600 transition-colors hover:bg-emerald-500/20 dark:border-emerald-800 dark:text-emerald-400"
                      onClick={() => onUnhideProject(project.name)}
                      title={t('tooltips.showProject')}
                    >
