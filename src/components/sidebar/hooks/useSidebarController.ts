...
const HIDDEN_PROJECTS_STORAGE_KEY = 'cloudcli-hidden-projects';

const loadHiddenSessionsByProject = (): HiddenSessionsByProject => {
  try {
    const raw = localStorage.getItem(HIDDEN_SESSIONS_STORAGE_KEY);
    if (!raw) return {};
    const parsed = JSON.parse(raw) as HiddenSessionsByProject;
    if (!parsed || typeof parsed !== 'object') return {};
    return Object.fromEntries(
      Object.entries(parsed).map(([projectName, sessionIds]) => [
        projectName,
        Array.isArray(sessionIds) ? sessionIds.filter((value): value is string => typeof value === 'string') : [],
      ]),
    );
  } catch {
    return {};
...
const persistHiddenSessionsByProject = (value: HiddenSessionsByProject) => {
  try {
    localStorage.setItem(HIDDEN_SESSIONS_STORAGE_KEY, JSON.stringify(value));
  } catch {
    // Ignore storage failures
  }
};

const loadHiddenProjects = (): HiddenProjects => {
  try {
...
const persistHiddenProjects = (value: HiddenProjects) => {
  try {
    localStorage.setItem(HIDDEN_PROJECTS_STORAGE_KEY, JSON.stringify(value));
  } catch {
    // Ignore storage failures
  }
};
...
export function useSidebarController({
  ...
  const [hiddenSessionsByProject, setHiddenSessionsByProject] = useState<HiddenSessionsByProject>(() =>
    loadHiddenSessionsByProject(),
  );
  const [expandedHiddenProjects, setExpandedHiddenProjects] = useState<Set<string>>(new Set());
  const [hiddenProjects, setHiddenProjects] = useState<HiddenProjects>(() => loadHiddenProjects());
  ...
  useEffect(() => {
    persistHiddenSessionsByProject(hiddenSessionsByProject);
  }, [hiddenSessionsByProject]);

  useEffect(() => {
    const archivedByProject: HiddenSessionsByProject = {};

    projects.forEach((project) => {
      const archivedIds = getAllSessions(project, additionalSessions)
        .filter((session) => Boolean((session as ProjectSession & { __continuationArchived?: boolean }).__continuationArchived))
        .map((session) => session.id);

      if (archivedIds.length > 0) {
        archivedByProject[project.name] = archivedIds;
      }
    });

    if (Object.keys(archivedByProject).length === 0) {
      return;
    }

    setHiddenSessionsByProject((prev) => {
      let changed = false;
      const next: HiddenSessionsByProject = { ...prev };

      Object.entries(archivedByProject).forEach(([projectName, archivedIds]) => {
        const merged = Array.from(new Set([...(prev[projectName] || []), ...archivedIds]));
        if (merged.length !== (prev[projectName] || []).length) {
          changed = true;
          next[projectName] = merged;
        }
      });

      return changed ? next : prev;
    });
  }, [additionalSessions, projects]);

  useEffect(() => {
    const syncHiddenSessions = () => {
      setHiddenSessionsByProject(loadHiddenSessionsByProject());
    };

    window.addEventListener('cloudcli:hidden-sessions-updated', syncHiddenSessions as EventListener);
    return () => {
      window.removeEventListener('cloudcli:hidden-sessions-updated', syncHiddenSessions as EventListener);
    };
  }, []);

  useEffect(() => {
    persistHiddenProjects(hiddenProjects);
  }, [hiddenProjects]);
  ...
  const filteredProjects = useMemo(
    () => filterProjects(sortedProjects, searchFilter).filter((project) => !hiddenProjects.includes(project.name)),
    [hiddenProjects, searchFilter, sortedProjects],
  );

  const hiddenProjectEntries = useMemo(
    () => sortedProjects.filter((project) => hiddenProjects.includes(project.name)),
    [hiddenProjects, sortedProjects],
  );
  ...
  const hideSession = useCallback((projectName: string, sessionId: string) => {
    setHiddenSessionsByProject((prev) => {
      const nextIds = Array.from(new Set([...(prev[projectName] || []), sessionId]));
      return { ...prev, [projectName]: nextIds };
    });
  ...
  const hideProject = useCallback((projectName: string) => {
    ...
    setHiddenProjectsExpanded(true);
  }, []);

  const unhideProject = useCallback((projectName: string) => {
    setHiddenProjects((prev) => prev.filter((name) => name !== projectName));
  }, []);

  const toggleHiddenProjectsSection = useCallback(() => {
    setHiddenProjectsExpanded((prev) => !prev);
  }, []);

  const unhideSession = useCallback((projectName: string, sessionId: string) => {
    setHiddenSessionsByProject((prev) => {
      const remaining = (prev[projectName] || []).filter((id) => id !== sessionId);
      if (remaining.length === 0) {
        const { [projectName]: _removed, ...rest } = prev;
        return rest;
      }
      return { ...prev, [projectName]: remaining };
    });
  }, []);

  const toggleHiddenSection = useCallback((projectName: string) => {
    setExpandedHiddenProjects((prev) => {
      const next = new Set(prev);
      if (next.has(projectName)) {
        next.delete(projectName);
