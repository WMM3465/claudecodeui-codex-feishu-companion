...
export const getSessionDate = (session: SessionWithProvider): Date => {
  ...
  if (session.__provider === 'codex') {
    const sortAnchor =
      typeof (session as { __sortAnchorTime?: string }).__sortAnchorTime === 'string'
        ? (session as { __sortAnchorTime?: string }).__sortAnchorTime || ''
        : '';
    return new Date(sortAnchor || session.createdAt || session.lastActivity || 0);
  }
...
export const getSessionName = (session: SessionWithProvider, t: TFunction): string => {
  const normalizedSummary = session.summary === 'New Session' ? '' : session.summary;
  const normalizedName = session.name === 'New Session' ? '' : session.name;

  if (session.__provider === 'cursor') {
    return normalizedSummary || normalizedName || t('projects.untitledSession');
  }

  if (session.__provider === 'codex') {
    return normalizedSummary || normalizedName || t('projects.newSession');
  }
...
export const getAllSessions = (
  ...
  const visibleCodexSessions = codexSessions;

  return [...claudeSessions, ...cursorSessions, ...visibleCodexSessions, ...geminiSessions].sort(
