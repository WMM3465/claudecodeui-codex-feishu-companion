...
import { applyCustomSessionNames, sharedWorkspaceDb } from './database/db.js';
          ...
          if (entry.sessionId) {
            ...
            if (entry.message?.role === 'user' && entry.message?.content) {
              ...
              if (Array.isArray(content) && content.length > 0 && content[0].type === 'text') {
                textContent = sanitizeCodexContentParts(content);
              } else if (typeof content === 'string') {
                textContent = sanitizeCodexVisibleUserMessage(content);
              }
...
async function deleteProject(projectName, force = false) {
  ...
  try {
    ...
    if (projectPath) {
      try {
        const codexSessions = await getCodexSessions(projectPath, { limit: 0, projectName });
...
function applyCodexContinuationStateToSessions(sessions, projectName = '') {
  if (!projectName || !Array.isArray(sessions) || sessions.length === 0) {
    return sessions;
  }

  const continuationState = sharedWorkspaceDb.getContinuationProjectState(projectName, 'codex');
  const continuationMap = continuationState?.chainsByRoot instanceof Map
    ? continuationState.chainsByRoot
    : new Map();
  const childSessionIds = continuationState?.childToRoot instanceof Map
    ? new Set(Array.from(continuationState.childToRoot.keys()))
    : new Set();

  const sessionById = new Map(sessions.map((session) => [session.id, session]));

  return sessions.reduce((acc, session) => {
    if (childSessionIds.has(session.id)) {
      return acc;
    }

    const chain = continuationMap.get(session.id) || [];
    if (chain.length === 0) {
      acc.push(session);
      return acc;
    }
    ...
    const latestSession = sessionById.get(latest.sessionId) || session;
    acc.push({
      ...session,
      id: session.id,
      name: latestSession.name || latestSession.summary || latestSession.title || session.name || session.summary || '共享线程',
      summary: latestSession.summary || latestSession.name || latestSession.title || session.summary || session.name || '共享线程',
      title: latestSession.title || latestSession.summary || latestSession.name || session.title || session.summary || session.name || '共享线程',
      __logicalRootSessionId: session.id,
      __activeSessionId: latest.sessionId,
      __continuationChainLength: chain.length,
      __continuationRepresentative: true,
      __sortAnchorTime: session.lastActivity || session.createdAt || latest.createdAt || '',
      __continuationLatestCreatedAt: latestSession.lastActivity || latest.createdAt || '',
      lastActivity: latestSession.lastActivity || latest.createdAt || session.lastActivity || session.createdAt || '',
...
async function getCodexSessions(projectPath, options = {}) {
  const { limit = 5, indexRef = null, projectName = '' } = options;
  try {
    ...
    const visibleSessions = applyCodexContinuationStateToSessions(sessions, projectName);
...
function isVisibleCodexUserMessage(payload) {
  ...
  if (!sanitizeCodexVisibleUserMessage(payload.message)) {
    return false;
  }
...
function sanitizeCodexVisibleUserMessage(message) {
  if (typeof message !== 'string') {
    return '';
  }
  const modernContinuationMatch = message.match(/<context_summary>\s*[\s\S]*?\s*<\/context_summary>[\s\S]*?<new_request>\s*([\s\S]*?)\s*<\/new_request>/i);
  if (modernContinuationMatch?.[1]) {
    return modernContinuationMatch[1].trim();
  }
  const legacyContinuationMatch = message.match(/<姘撴菠閳ュ顬犳偢鈹熴剢鎹欐崡锔瑰妷鐐?\s*[\s\S]*?\s*<\/姘撴菠閳ュ顬犳偢鈹熴剢鎹欐崡锔瑰妷鐐?[\s\S]*?<蹇欓垾鎾€鎳婎灒鈾€鈧伝顭婎嚪璁课叉墎鈧?\s*([\s\S]*?)\s*<\/蹇欓垾鎾€鎳婎灒鈾€鈧伝顭婎嚪璁课叉墎鈧?/i);
  if (legacyContinuationMatch?.[1]) {
    return legacyContinuationMatch[1].trim();
  }
  const userRequestMatch = message.match(/<user_request>\s*([\s\S]*?)\s*<\/user_request>/i);
  ...
  return message
    ...
    .replace(/<new_request>/gi, '')
    .replace(/<\/new_request>/gi, '')
    .replace(/<\/?user_request>/gi, '')
    .replace(/<姘撴菠閳ュ顬犳偢鈹熴剢鎹欐崡锔瑰妷鐐?[\s\S]*?<\/姘撴菠閳ュ顬犳偢鈹熴剢鎹欐崡锔瑰妷鐐?/gi, '')
    .replace(/<\/?蹇欓垾鎾€鎳婎灒鈾€鈧伝顭婎嚪璁课叉墎鈧?/gi, '')
    .trim();
}


function sanitizeCodexContentParts(contentParts) {
  if (!Array.isArray(contentParts)) {
    return '';
  }

  const mergedText = contentParts
    .filter((part) => (part.type === 'input_text' || part.type === 'text') && typeof part.text === 'string')
    .map((part) => part.text)
    .join(' ');

  return sanitizeCodexVisibleUserMessage(mergedText);
}
        ...
        try {
          ...
          if (entry.type === 'event_msg' && isVisibleCodexUserMessage(entry.payload)) {
            ...
            if (entry.payload.message) {
              lastUserMessage = sanitizeCodexVisibleUserMessage(entry.payload.message);
            }
          }
        ...
        try {
          ...
          if (entry.type === 'event_msg' && isVisibleCodexUserMessage(entry.payload)) {
            messages.push({
              ...
              message: {
                role: 'user',
                content: sanitizeCodexVisibleUserMessage(entry.payload.message)
              }
            });
          }
